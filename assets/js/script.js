...
(function ($) {
  window.ndsIsInViewport = function (el) {
    var percentVisible = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0.25;
    ...
    var visibleTop = Math.max(0, 0 - rect.top);
    var visibleBottom = Math.max(0, rect.bottom - windowHeight);
    var visibleHeight = elementHeight - visibleTop - visibleBottom;
  ...
  function ndsSiteMenu() {
    $('#primary-menu-toggle').click(function (e) {
      ...
      var menuWrapper = $('.nds-mobile-menu-wrapper');
      if ($('body').hasClass('nds-mobile-menu-open')) {
        var headerHeight = $('.nds-site-header').outerHeight() - 1;
    ...
    $('.nds-primary-menu a[data-identifier]').hover(function () {
      var $this = $(this);
    ...
    $('.nds-mobile-primary-menu a[data-identifier]').on('click.ndsMobileMega', function (e) {
      ...
      var identifier = $clickedLink.data('identifier');
      ...
      $(".nds-mobile-megamenu-item[data-identifier=\"".concat(identifier, "\"]")).addClass('active');
  ...
  window.ndsBlockSection = function () {
      ...
      $(scope).find('.nds-lazyvideo').each(function () {
        ...
        if ($el.is('video') && $el.data('src')) {
          ...
          video.play()["catch"](function (err) {
            return console.log('Video play error:', err);
          });
        }
      });
    }
  ...
  window.ndsYoutubeEmbed = function () {
    var scope = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : document;
    ...
    var $blocks = $(scope).find('.nds-youtube-embed');
    if (!$blocks.length) return;
    $blocks.each(function () {
      var $block = $(this);
      var $wrapper = $block.find('.yt-wrapper');
      var $container = $block.find('.yt-container');
      var videoId = $container.data('youtube-id');
      if (!videoId) return;
      var state = {
        loaded: false,
        played: false
      ...
      function loadYouTubeIframe() {
        ...
        var params = ["autoplay=".concat(autoplay ? 1 : 0), 'enablejsapi=1', 'rel=0', 'playsinline=1', 'modestbranding=1', 'showinfo=0'].join('&');
        var iframe = "\n                        <iframe\n                        src=\"https://www.youtube.com/embed/".concat(videoId, "?").concat(params, "\"\n                        frameborder=\"0\"\n                        allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\"\n                        allowfullscreen\n                        class=\"w-full h-full\"\n                        ></iframe>\n                        ");
      ...
      if ('IntersectionObserver' in window) {
        ...
        var ns = '.ndsYT-' + Math.random().toString(36).slice(2);
  ...
  window.ndsYoutubePauseAll = function () {
    var scope = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : document;
    $(scope).find('.nds-youtube-embed').each(function () {
      var _$$data;
      (_$$data = $(this).data('ndsYT')) === null || _$$data === void 0 || _$$data.pause();
    });
  };
  window.ndsYoutubeStopAll = function () {
    var scope = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : document;
    $(scope).find('.nds-youtube-embed').each(function () {
      var _$$data2;
      (_$$data2 = $(this).data('ndsYT')) === null || _$$data2 === void 0 || _$$data2.stop();
    });
  };
  ...
  window.ndsTeamMembers = function () {
    var scope = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : document;
    var $scope = $(scope);
    ...
    function initTeamMemberExpand() {
      $scope.find('.nds-btn-expand-description').off('click.ndsTeam').on('click.ndsTeam', function (e) {
        e.preventDefault();
        var $button = $(this);
        var $teamMemberItem = $button.closest('.nds-team-member-item');
        if (!$teamMemberItem.length) {
          return;
        }
        var $description = $teamMemberItem.find('.nds-team-member-description');
        if (!$description.length) {
          return;
        }
        var isExpanded = $teamMemberItem.hasClass('expanded');
        if (isExpanded) {
          // Collapse
          ...
          var expandText = $button.data('expand-text') || 'Read More';
          ...
          $description.css('max-height', $description[0].scrollHeight + 'px');
          var collapseText = $button.data('collapse-text') || 'Read Less';
    ...
    function initTeamMemberSwiper() {
      $scope.find('.nds-teams-featured .swiper').each(function () {
        ...
        var blockContainer = $swiper.closest('.nds-teams-featured');
        if (!swiperId || $swiper.data('swiper-initialized')) {
          return;
        }

        // Get columns from the closest block data attribute or default to 3
        var $block = $swiper.closest('.nds-teams-featured');
        var columns = $block.data('columns') || 3;
    ...
    function initTeamMemberGridSwiper() {
      $scope.find('.nds-teams-grid .swiper').each(function () {
        var $swiper = $(this);
        var swiperId = $swiper.attr('id');
        var blockContainer = $swiper.closest('.nds-teams-grid');
        if (!swiperId || $swiper.data('swiper-initialized')) {
          return;
        }
        var $block = $swiper.closest('.nds-teams-grid');
        var swiperInstance = new Swiper('#' + swiperId, {
          slidesPerView: 1,
          spaceBetween: 13,
          loop: false,
    ...
    function initTeamMemberLocationSwiper() {
      $scope.find('.nds-teams-location .swiper').each(function () {
        var $swiper = $(this);
        var swiperId = $swiper.attr('id');
        var blockContainer = $swiper.closest('.nds-teams-location');
        if (!swiperId || $swiper.data('swiper-initialized')) {
          return;
        }

        // Get columns from the closest block data attribute or default to 3
        var $block = $swiper.closest('.nds-teams-location');
        var columns = $block.data('columns') || 3;
        var swiperInstance = new Swiper('#' + swiperId, {
          slidesPerView: 1,
          spaceBetween: 13,
  ...
  window.ndsTestimonials = function () {
    var scope = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : document;
    var $scope = $(scope);
    ...
    function initTestimonialsSwiper() {
      $scope.find('.nds-testimonials .testimonials-swiper').each(function () {
        var $swiper = $(this);
        var $block = $swiper.closest('.nds-testimonials');
        ...
        var autoplay = $block.data('autoplay') || false;
  ...
  window.ndsTimelineSlider = function () {
    var scope = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : document;
    var $scope = $(scope);
    ...
    function initTimelineSwiper() {
      $scope.find('.nds-timeline-slider .nds-timeline-slider-wrapper').each(function () {
        var $swiper = $(this);
        var $block = $swiper.closest('.nds-timeline-slider');
        var blockId = $block.data('block-id');
        if (!blockId || $swiper.data('swiper-initialized')) {
          return;
        }
        ...
        var itemsPerSlide = $block.data('items-per-slide') || 3;
        var forceLineFullwidth = $block.data('force-line-fullwidth') || false;
        var swiperInstance = new Swiper($swiper[0], {
          slidesPerView: 1,
          spaceBetween: 34,
          speed: 1500,
    ...
    function initTimelineExpand() {
      $scope.find('.nds-timeline-slider .nds-expandable').each(function () {
        var $slide = $(this);
        var $shortText = $slide.find('.nds-timeline-slide-short-text');
        var $expandContent = $slide.find('.nds-timeline-slide-expand-content');
  ...
  window.ndsFaqAccordion = function () {
    var SCOPE_SELECTOR = ".block-faqs"; // change to ".block-faqs .group" to scope per column
    var PER_GROUP = false; // set true if you want each .group independent

    function getScopes() {
      if (PER_GROUP) {
        return Array.from(document.querySelectorAll("".concat(SCOPE_SELECTOR, " .group")));
      }
    ...
    function attachListenerToDetail(detail, scopeRoot) {
      ...
      detail.addEventListener("toggle", function (e) {
        ...
        var others = Array.from(scopeRoot.querySelectorAll("details"));
    ...
    function scanAndAttach() {
      ...
      scopes.forEach(function (scopeRoot) {
        var details = Array.from(scopeRoot.querySelectorAll("details"));
        details.forEach(function (d) {
          return attachListenerToDetail(d, scopeRoot);
        });
      });
    }
            ...
            if (m.type === "childList") {
              m.addedNodes.forEach(function (node) {
                ...
                if (node.matches && node.matches("details")) {
                  ...
                  node.querySelectorAll && node.querySelectorAll("details").forEach(function (d) {
                    return attachListenerToDetail(d, scopeRoot);
                  });
                }
              });
  ...
  window.ndsTeamGrid = function () {
    var $grids = $('.team-grid-wrapper');
    $grids.each(function () {
      var $wrapper = $(this);
      var $grid = $wrapper.find('.team-grid');
      var $filters = $wrapper.find('.team-filters select');
      var $loadingOverlay = $wrapper.find('.loading-overlay');
      ...
      function initMobileSwiper() {
        if (window.innerWidth <= 767 && !swiperInstance) {
          ...
          var _$wrapper = $('<div class="swiper-wrapper"></div>');
          $grid.find('.team-member-item').each(function () {
            $(this).wrap('<div class="swiper-slide"></div>');
          });
          $grid.wrapInner(_$wrapper);
      ...
      function initTeamSliderSwiper() {
        var $sliderContainer = $wrapper.find('.nds-team-slider-container');
        ...
        $sliderContainer.find('.swiper').each(function () {
          ...
          if ($swiper.data('swiper-initialized')) {
            var existingInstance = $swiper.data('swiper-instance');
        ...
        $.ajax({
          ...
          success: function success(response) {
            if (response.success) {
              ...
              var $mobileSwiper = $wrapper.find('.nds-team-members-slider .swiper-wrapper');
              ...
              var $teamSliderWrapper = $wrapper.find('.nds-team-slider-container .swiper-wrapper');
  ...
  window.ndsArticlesArchive = function () {
    var $archives = $('.nds-articles-archive');
    $archives.each(function () {
      var $wrapper = $(this);
      var $grid = $wrapper.find('.nds-articles-archive-posts-grid');
      var $categoryFilter = $wrapper.find('[data-filter="category"]');
      var $searchInput = $wrapper.find('[data-filter="search"]');
      var $searchForm = $wrapper.find('.nds-articles-search-form');
      var $loadMoreBtn = $wrapper.find('.nds-load-more-btn');
      var $loadingOverlay = $wrapper.find('.loading-overlay');
      var currentPage = 1;
      var currentCategory = '';
      var currentSearch = '';
      ...
      // Get block data
      var postsPerPage = parseInt($wrapper.data('posts-per-page')) || 6;
      ...
      function loadPosts() {
        ...
        $.ajax({
          ...
          error: function error(xhr, status, _error) {
            console.error('Articles filter AJAX error:', _error);
            resetLoadingState();
          }
        });
      }
  ...
  window.ndsSimpleGoogleMap = function () {
    var $maps = $('.nds-google-map-simple');
    $maps.each(function () {
      var $mapContainer = $(this);
      var blockId = $mapContainer.data('block-id');
      var map = null;
      var markers = [];
      var geocoder = null;
      var infoWindow = null;
      ...
      function addLocationMarkers(locations) {
        ...
        locations.forEach(function (location, index) {
          geocodeAddress(location.full_address, location, index, function () {
            markersAdded++;
      ...
      function geocodeAddress(address, locationData, index, callback) {
        ...
        geocoder.geocode({
          ...
          if (status === 'OK' && results[0]) {
            ...
            var markerScale = locationData.is_hq ? 1.25 : 0.75;
            ...
            var infoContent = "\n                                    <div class=\"nds-map-info-window\">\n                                          <p class=\"text-sm font-normal\">".concat(locationData.name, "</p>\n                                          <p class=\"text-sm\">").concat(locationData.address, "</p>\n                                          <p class=\"text-sm\">").concat(locationData.postcode, "</p>\n                                          ").concat(locationData.phone_number ? "<p class=\"text-sm\">Phone: ".concat(locationData.phone_number, "</p>") : '', "\n                                    </div>\n                              ");
  ...
  window.ndsLocationsSearch = function () {
    var $locators = $('.nds-locations-search');
    $locators.each(function () {
      var $wrapper = $(this);
      var $searchForm = $wrapper.find('.nds-postcode-search-form');
      var $searchInput = $wrapper.find('.nds-postcode-input');
      var $clearBtn = $wrapper.find('.nds-clear-search-btn');
      var $locationsList = $wrapper.find('.nds-locations-container');
      var $resultsCount = $wrapper.find('.nds-results-count');
      var $resultsText = $wrapper.find('.nds-results-text');
      var $loadingOverlay = $wrapper.find('.nds-loading-overlay');
      var $mapContainer = $wrapper.find('.nds-google-map');
      var map = null;
      var markers = [];
      var geocoder = null;
      var infoWindow = null;
      ...
      var radiusKm = null;
      var allLocationsGeocoded = false;

      // Get locations data and radius from the map container
      try {
        allLocations = JSON.parse($mapContainer.attr('data-locations') || '[]');
        filteredLocations = _toConsumableArray(allLocations);
      ...
      function geocodeAddress(address, locationData, index, callback) {
        ...
        geocoder.geocode({
          ...
          if (status === 'OK' && results[0]) {
            ...
            var originalLocation = allLocations.find(function (loc) {
              return loc.id === locationData.id && loc.full_address === locationData.full_address;
            });
            if (originalLocation) {
            ...
            // Create info window content
            var infoContent = "\n                                    <div class=\"nds-map-info-window\">\n                                          <p class=\"text-sm font-normal\">".concat(locationData.title, "</p>\n                                          <p class=\"text-sm\">").concat(locationData.address, "</p>\n                                          <p class=\"text-sm\">").concat(locationData.postcode, "</p>\n                                          ").concat(locationData.distance ? "<p class=\"hidden\">".concat(locationData.distance, "km away</p>") : '', "\n                                    </div>\n                              ");
      ...
      function filterLocationsByRadius(searchLat, searchLng, locations) {
        ...
        console.log("Filtering locations within ".concat(radiusKm, "km of"), searchLat, searchLng);
        console.log('Total locations to check:', locations.length);
        var filteredResults = locations.filter(function (location) {
          if (!location.coordinates) {
            console.log("Location ".concat(location.title, " has no coordinates, skipping"));
            return false;
          }
          var distance = calculateDistance(searchLat, searchLng, location.coordinates.lat, location.coordinates.lng);
          location.distance = Math.round(distance * 10) / 10; // Round to 1 decimal place
          var withinRadius = distance <= radiusKm;
          console.log("".concat(location.title, ": ").concat(distance.toFixed(1), "km away - ").concat(withinRadius ? 'INCLUDED' : 'EXCLUDED'));
          return withinRadius;
        });
        console.log("Radius filtering complete: ".concat(filteredResults.length, " locations within ").concat(radiusKm, "km"));
      ...
      function highlightLocationItem(locationId) {
        ...
        $locationsList.find("[data-location-id=\"".concat(locationId, "\"]")).addClass('active');
      }
      ...
      function filterLocationsByPostcode(searchPostcode) {
        ...
        var search = searchPostcode.toLowerCase().replace(/\s+/g, '');
        return allLocations.filter(function (location) {
          var locationPostcode = location.postcode.toLowerCase().replace(/\s+/g, '');
          var locationAddress = location.address.toLowerCase();

          // Match postcode or address
          return locationPostcode.includes(search) || locationAddress.includes(searchPostcode.toLowerCase());
        });
      }

      // Update locations display
      function updateLocationsDisplay(locations) {
        var searchContext = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
        ...
        if (searchContext && searchContext.postcode) {
          ...
          $resultsCount.text("".concat(searchContext.cityTown ? searchContext.cityTown + ', ' : '').concat(searchContext.postcode));
        } else {
      ...
      function searchByPostcode(postcode) {
        ...
        setTimeout(function () {
          var results = filterLocationsByPostcode(postcode);
          ...
          if (geocoder) {
            geocoder.geocode({
              ...
              if (status === 'OK' && geocodeResults[0]) {
                ...
                var addressComponents = geocodeResults[0].address_components;
                var cityTown = '';
                ...
                try {
                  for (_iterator.s(); !(_step = _iterator.n()).done;) {
                    ...
                    if (component.types.includes('locality') || component.types.includes('postal_town') || component.types.includes('administrative_area_level_2')) {
                ...
                if (radiusKm) {
                  ...
                  var _waitForGeocoding = function waitForGeocoding() {
                    if (allLocationsGeocoded) {
                      console.log('All locations geocoded, proceeding with radius filtering');
                      var searchPosition = geocodeResults[0].geometry.location;
                      // Apply radius filtering to ALL locations, not just postcode matches
                      finalResults = filterLocationsByRadius(searchPosition.lat(), searchPosition.lng(), allLocations // Use ALL locations for radius filtering
                      );
                      updateLocationsDisplay(finalResults, searchContext);
                      $loadingOverlay.addClass('hidden');
                    } else {
                      console.log('Waiting for geocoding to complete...');
                      setTimeout(_waitForGeocoding, 500);
                    }
                  };
                  _waitForGeocoding();
            ...
            updateLocationsDisplay(results, {
              postcode: postcode
            });
            $loadingOverlay.addClass('hidden');
          }
      ...
      $searchForm.on('submit', function (e) {
        e.preventDefault();
        var postcode = $searchInput.val().trim();
      ...
      $wrapper.on('click', '.nds-location-item', function () {
        ...
        var marker = markers.find(function (m) {
          return m.uniqueLocationId === locationId;
        });
        if (marker && map) {
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunknds"] = self["webpackChunknds"] || [];
...
/******/ 	__webpack_require__.O(undefined, ["css/editor-style","css/nth"], () => (__webpack_require__("./resources/js/app.js")))
/******/ 	__webpack_require__.O(undefined, ["css/editor-style","css/nth"], () => (__webpack_require__("./resources/css/app.css")))
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, ["css/editor-style","css/nth"], () => (__webpack_require__("./resources/css/editor-style.css")))
